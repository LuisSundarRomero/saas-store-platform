...
const ESTADOS = ['todos', 'pendiente', 'atendido']
const POR_PAGINA = 10
...
export default async function ReclamacionesPage({ searchParams }: Props) {
  ...
  const atendidos = todos.filter((r) => r.estado === 'atendido').length
  const esteMes = todos.filter((r) => {
  ...
  function buildUrl(params: Record<string, string>) {
    ...
    return `/admin/reclamaciones?${p.toString()}`
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Gestiona los reclamos y quejas de tus clientes</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <ReclamacionesBuscador defaultValue={q} estado={estado} />
      </div>
      ...
      {q && (
        <p className="text-sm text-gray-500 mb-3">
          ...
          <Link href="/admin/reclamaciones" className="ml-2 text-red-500 hover:underline text-xs">Limpiar</Link>
        </p>
      )}

      <ReclamacionesTable reclamaciones={reclamacionesPagina} />

      {/* Paginación */}
      {totalPaginas > 1 && (
        <div className="mt-6 flex items-center justify-between">
          <p className="text-sm text-gray-400">
            Mostrando {(page - 1) * POR_PAGINA + 1}–{Math.min(page * POR_PAGINA, totalFiltrados)} de {totalFiltrados} reclamaciones
          </p>
